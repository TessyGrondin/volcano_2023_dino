...
void create_monster(all_t *all, sfVector2i pos)
{
    ...
    if (all->map.tiles[SPONE][index] == 1) {
        for (; all->npc[i].sp.sp != NULL; i++);
        color = rand() % 2;
        if (color == 1)
            all->npc[i] = entit_create(all->clock, P_NPC1, "npc1");
        else
            all->npc[i] = entit_create(all->clock, P_NPC2, "npc2");
        all->npc[i + 1].sp.sp = NULL;
        sfSprite_setPosition(all->npc[i].sp.sp, set);
        return;
    }
    // npc_range(all, index);
}
...
void map_borders(all_t *all)
{
    ...
    for (int i = 0; all->npc[i].sp.sp != NULL; i++) {
        pos = sfSprite_getPosition(all->npc[i].sp.sp);
        pos.x = (pos.x < 0) ? 0 : pos.x;
        pos.x = (pos.x > right) ? right : pos.x;
        pos.y = (pos.y < 0) ? 0 : pos.y;
        pos.y = (pos.y > down) ? down : pos.y;
        sfSprite_setPosition(all->npc[i].sp.sp, pos);
    }
